...
REPORTS_DIR = "reports"
# NOTE: These directories will be created in the temporary Serverless Function environment
os.makedirs(REPORTS_DIR, exist_ok=True)
os.makedirs("live_json", exist_ok=True)

# --- FIREBASE SETUP (Restored Logic) ---
try:
    # Looks for Environment Variable 'FIREBASE_CREDENTIALS' first
    ...
    if os.path.exists(CREDENTIALS_PATH) or os.environ.get('FIREBASE_CREDENTIALS'):
        ...
        if CREDENTIALS_PATH and CREDENTIALS_PATH.startswith('{'):
            ...
            # Load credentials from the environment variable (Vercel/Render method)
            cred = credentials.Certificate(json_lib.loads(CREDENTIALS_PATH))
        else:
            # Use local file path (for local testing/PythonAnywhere fallback)
